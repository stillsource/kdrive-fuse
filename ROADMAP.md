...
### Chunked upload (> 16 MB) with parallel connections
Files larger than 16 MB upload via the kDrive upload-session flow (`POST /upload/session/start` → `POST /upload/session/{token}/chunk` × N → `POST /upload/session/{token}/finish`, `DELETE /upload/session/{token}` to cancel). 16 MB chunks, per-chunk retry on transient failures, cancel-on-failure to release the partial session, and a `ChunkHasher` hash-of-hashes for `total_chunk_hash`. Smaller files keep the single-shot path.

### Parallel chunk uploads (`KDRIVE_UPLOAD_PARALLELISM`)
The chunks of one upload session are uploaded concurrently on the stdlib default HTTP transport (whose connection pool opens multiple connections to the upload host, aggregating bandwidth — the same approach the official desktop client uses). Measured live on a 200 MB file: ~33 MB/s sequential → ~47 MB/s at parallelism 6. The default parallelism is 4; tune via `KDRIVE_UPLOAD_PARALLELISM`. Bodies that implement `io.ReaderAt` (e.g. `*os.File`, `*bytes.Reader`) get the parallel path; other readers fall back to sequential. The `total_chunk_hash` is always assembled in chunk order regardless of upload order (pre-allocated hash slice, indexed by chunk number). Memory is bounded to parallelism × chunkSize (4 × 16 MB = 64 MB at defaults). (Note: the upload transport is intentionally left as the stdlib default — an earlier attempt to force HTTP/1.1 broke the connection to the upload host with "use of closed network connection".)
